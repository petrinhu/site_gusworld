...
| `NOME-REVISTA` | **W2b** | Decisão | ★ **A revista não tem nome** — e ele apareceu no texto que o líder quer postar no X (*"saiu edição nova de [nome]"*). **O nome decide o tom de tudo que vier depois.** Sessão "cara e alcance". | Alta | `SERP-PESQUISA`, `OBJETIVO-SITE` | JS 3 · líder | ⏳ Pendente | — |
...
| `DOMINIO-DNS` | **W10** | Decisão | **Domínio indefinido** por restrição declarada. Registro + DNS. | Baixa | `D-GO-LIVE` | JS 3 · líder | ⛔ Bloqueado (autorização do líder) | — |
| `D-GO-LIVE` | **W10** | Decisão | **Produção NÃO AUTORIZADA.** Go/no-go do líder + autorização expressa de push **naquela ocasião** (autorização anterior não vale). | Baixa | `DEPLOY-STAGING`, `AUD-SPOILER`, `AUD-LICENCA`, `AUD-IA`, `AUD-LGPD`, `AUD-SEO` | JS 1 · líder | ⛔ Bloqueado (autorização do líder) | — |
...
- **Sessão do líder (WIP = 1):** escolher **uma** das duas pautas de W2 — **"direitos e exposição"** (`SPOILER-POLICY`, `D-LICENCA`, `D-LIVROS-IA`, `D-FIGURAS`, `D-DOACAO-PAYPAL`, `AI-DISC-ESCOPO`) ou **"cara e alcance"** (`NOME-REVISTA`, `D-NOME-SERP`, `D-IDENTIDADE`, `MODERACAO`, `RAGE-ISSUES`).

**Recomendação:** **"cara e alcance" primeiro**, e dentro dela o **`NOME-REVISTA`** — a revista não tem nome, o nome já apareceu no texto que ele quer postar no X, e ele **decide o tom de tudo que vier depois**. Mas `SERP-PESQUISA` roda antes (sem o dado, a conversa não fecha e vira duas).
