...
- **Uma peça exige servidor.** O poll do cupom (`/api/cupom-voto.php`) precisa **persistir voto sem terceiro** (regra `ZERO-DADO`/zero-terceiro: nada de analytics nem serviço externo; a persistência é um cookie funcional próprio, `glyfesse_votou`, que não identifica quem votou). Persistir estado no lado do cliente não conta como poll. Isso, sozinho, **força um runtime no servidor** — não há como ser 100% estático.
...
- **Zero terceiro por padrão** (`D-ANALYTICS`/`ZERO-DADO`): fontes e assets self-hosted; sem CDN de fonte externa, sem analytics de terceiro. O único cookie do site é o do voto do cupom (`glyfesse_votou`), funcional e de primeira parte, não um rastreador.
...
6. **Zero terceiro por padrão**: fontes e assets servidos do próprio domínio; sem analytics de terceiro, sem CDN externo. Exceção conhecida e aceita: o cookie funcional do voto do cupom (`glyfesse_votou`), que não sai do próprio domínio e não identifica ninguém.
