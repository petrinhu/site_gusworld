...
> `GATE-SPOILER`.
> ⛔ **Tom obrigatório: crédito técnico seco.** A ideia é dele; a peça credita sem infantilizar.
...
### Por que esta carta e não outra (prova de que a trava do roster não foi tocada)

**A carta canonicamente "primeira" do jogo, por decisão do líder, tem nome — e é exatamente a que a
pauta manda evitar.** `docs/design/mecanicas/save-por-local.md:60` registra, verbatim: *"[ela] é,
por decisão do líder canonizada em 2026-07-18, a primeira carta especial do jogador (...) e por isso
a de maior apego emocional de toda a coleção."* Essa carta é um dos 20 mestres do roster, e nomeá-la
é o gatilho exato de `SPOILER-CARTA-FARADAY` descrito na pauta. **Por isso a peça não fala dela.**

Em vez disso, a peça reconta a **origem da moldura** (`docs/design/card-frame-spec.md`, decisões do
líder de 2026-07-09) — o desenho estrutural que **toda** carta do jogo usa, mestre ou comum,
independente de qual nome carrega. Conferido explicitamente, por grep, que nenhum destes termos
aparece no texto final (pt nem en): nenhum dos 20 nomes do roster (`docs/design/roster-analogos/`),
nenhum ID de carta (`ELM-`, `FIS-`, `MAT-`, `CMP-`, `OCU-`, `ECO-`, `CAP-`), a palavra "Faraday", a
palavra "gaiola"/"cage", "PEM", "Sterling", "Codex" e "Tusk". A peça também não nomeia nenhuma cor de
domínio nem enumera os seis campos de conhecimento — generalizado como "qualquer área do
conhecimento", ainda mais conservador do que a pauta exigia.

**Fonte factual de cada beat, verificada em `docs/design/card-frame-spec.md`:**

| Beat do texto | Fonte |
| :--- | :--- |
| Moldura decidida 09/07/2026, uma estrutura para todas as cartas | `card-frame-spec.md`, cabeçalho "STATUS: PROPOSTA (design aprovado pelo criador 2026-07-09...)" |
| 1ª tentativa colorida direto (`pixellab-frame-cyan-v1.png`) | mesma seção, "teste `create_ui_asset` provou que sai lindo" |
| 2ª tentativa neutra, arco corrigido (`pixellab-frame-neutral-arched-v2.png`) | seção "Recolor por domínio — mecanismo confirmado" |
| Tingir tudo (multiply) estragava o ouro; corrigido para tingir só as runas (luminance-key, threshold 0,70) | seção "DECISÃO (criador, 2026-07-09, pós-teste empírico)" + "FECHADO (2026-07-09)" |
| Uma amostra completa montada ponta a ponta pra validar o caminho inteiro | seção "CARTA-AMOSTRA (...) — CANONIZADA (criador, 2026-07-09)" — **o nome real da amostra foi lido pela pesquisa e deliberadamente omitido do texto**, pelo motivo acima |
| Fecho em 14/07 do mecanismo de efeito (não descrito) | `HISTORICO_GUS_ECOSSISTEMA.md`: *"2026-07-14 · ADR-016 motor de efeitos das cartas = executor data-driven techMagic (não VM)"* — citada só como data, nunca como mecanismo, cumprindo o corte da lente |

⚠️ **Isto não é a mesma coisa que a pauta pediu ao pé da letra.** A pauta fala em "a primeira ideia
de carta e o sprite do qual ela nasceu" — a pesquisa não achou uma carta específica, não-mestre, cuja
origem visual seja datada e documentada dentro da janela sem tocar o roster. O que existe, bem
documentado e dentro da janela (09/07, antes do fecho de 14/07), é a origem da **moldura**: o
primeiro objeto visual que qualquer carta, de qualquer nome, precisou ter antes de existir. Trato
isso como resposta honesta à lente ("um único exemplar, do desenho ao efeito fechado"), não como
desvio dela — mas registro aqui a diferença para o `GATE-LENTE` confirmar ou pedir ajuste, em vez de
presumir aprovação.

### Cartas comuns como alternativa considerada e descartada

A pesquisa também achou 5 cartas **comuns** (não-mestres, sem trava de roster): Pulso Elétrico, Raiz
Curativa, `eco_sonico`, `fenda_criptica`, `impacto_cinetico`
(`docs/design/mecanicas/cartas/_INDEX.md`). Descartei essa rota porque nenhuma delas tem sprite ou
arte de origem documentada — no repositório elas existem só como registro de dados (`.gw.card`),
sem imagem própria, sem data de criação anterior a meados de julho, e sem nenhuma "primeira versão"
para contar. Contar a origem visual de uma carta sem imagem nenhuma seria inventar o que a pesquisa
não achou.

### Pendência para o líder

Confirmar se a rota escolhida (a moldura, sem nomear a amostra) cumpre a intenção do Gus Dragon, ou
se ele prefere esperar a matéria própria do roster para contar "a primeira carta" com nome — antes
do `GATE-SPOILER` fechar esta peça.
