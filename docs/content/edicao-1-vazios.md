# Glyfesse #1 — Os 6 vazios-com-graça

> A piada honesta das seções que não têm material em maio: o jogo mal existe, só a lore.
> Vozes: **Gus** (`gus@glyfesse>`, pensamento em `//`) nas seções 5-9; **Pyotor** (`root@glyfesse>`) na 12.
> Papel = acento pt-br completo; tokens de código (`<NULL>`, ícone quebrado, prompts) em ASCII.
> Sem em-dash, profanidade zero. Sem spoiler de lore/mecânica/personagem.

---

## 5 — A NOTA

### pt-BR

`gus@glyfesse>` a nota

Como dar nota a um jogo que ainda não é jogo? Assim:

Gráficos: `[ícone de imagem quebrada]`
Jogabilidade: `<NULL>`
Texto: 10. É tudo que tem.

`// a nota sobe a cada edição. começa onde dá.`

### EN

`gus@glyfesse>` the score

How do you score a game that isn't a game yet? Like this:

Graphics: `[broken image icon]`
Gameplay: `<NULL>`
Writing: 10. It's all there is.

`// the score climbs every edition. it starts where it can.`

---

## 6 — GALERIA DE BUGS

### pt-BR

`gus@glyfesse>` galeria de bugs

Nenhum bug esta edição. Não porque o código é perfeito: porque não existe código. Volte na #3.

### EN

`gus@glyfesse>` gallery of bugs

No bugs this edition. Not because the code is perfect: because there is no code. Come back in #3.

---

## 7 — CEMITÉRIO DE IDEIAS MORTAS

### pt-BR

`gus@glyfesse>` cemitério de ideias mortas

O cemitério ainda está vazio. Em maio, tudo é possível. Os mortos (o Godot, o 3D, o Qt) chegam com o código, de junho em diante.

### EN

`gus@glyfesse>` graveyard of dead ideas

The graveyard is still empty. In May, everything is still possible. The dead (Godot, the 3D, the Qt) arrive with the code, from June on.

---

## 8 — DETONADO

### pt-BR

`gus@glyfesse>` detonado

Detonar o quê? Não tem jogo. Páginas em branco e um AGUARDE.

### EN

`gus@glyfesse>` walkthrough

Walk through what? There's no game. Blank pages and a STAND BY.

---

## 9 — ERRATA + CARTAS

### pt-BR

`gus@glyfesse>` errata + cartas

Sem errata: é a primeira edição, ainda não deu tempo de errar em público. Sem cartas: ninguém escreveu. Seja o primeiro (o e-mail está no rodapé).

### EN

`gus@glyfesse>` errata + letters

No errata: it's the first edition, there hasn't been time to be wrong in public yet. No letters: nobody has written. Be the first (the e-mail is in the footer).

---

## 12 — PRÓXIMOS LANÇAMENTOS

> ⚠️ Voz **Pyotor** (`root@glyfesse>`), o editor. Honesto, não promete data. Sem citar mecânica/mestre/carta.

### pt-BR

`root@glyfesse>` próximos lançamentos

A intenção é semanal. Mais ou menos... 'devezenquandal' fica mais fácil de afirmar.

### EN

`root@glyfesse>` what's next

The intention is weekly. More or less... 'every-so-oftenly' is easier to promise.
