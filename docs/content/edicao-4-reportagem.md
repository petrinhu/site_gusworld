...
> sete dias no total.
> **A tese é DISTÂNCIA CONTADA, não lista de marcos.** Cada fato que não encurta rosto→voz ficou de fora
...
### Conferências do redator (autodeclaradas — ainda faltam GATE-CONTEUDO e copyedit)

| Checagem | Resultado |
| :--- | :--- |
| Tamanho pt-BR | **521 palavras** (#1 = 383, #2 = 472, #3 = 372 — a #4 é a primeira XL, dentro do alvo 500-700 pedido) |
| Nome de tecnologia real | **nenhum** (varredura por Godot, C#, Qt, SDL, PixelLab, POCO, ADR, glintfx, Unity, Unreal — nenhum aparece; "placeholder", "engine"/"motor" e "2D" são os únicos termos técnicos, e são genéricos, no precedente da #3) |
| Rótulo clínico | **nenhum** |
| Cronologia | abre em 24/jun (quarta), passa por 6/jul (segunda), fecha em 21/jul (terça); nenhuma citação a 22/jul em diante; a única referência anterior é uma linha de ponte a 23/jun (terça, já contado pela #3), sem repetir conteúdo dela |
| Nome de batismo de criança | **nenhum** — só "Gus" (implícito, narrador) e "Seu Bertoldo Caím" |
| Fala verbatim do NPC de lore | **nenhuma** — a primeira fala do Bertoldo Caím é citada como existente e deliberadamente NÃO reproduzida ("ninguém além de mim sabe ainda qual foi"), por ele ser NPC de lore sujeito a `GATE-SPOILER` (risco 3 da pauta) |
| Piscada pro leitor / ironia dramática | **nenhuma encontrada na releitura** — toda frase é afirmável de dentro de 21/jul com o que se sabia então; a peça não toca glintfx (reservado à Reportagem extra) nem antecipa nada de 22/jul em diante |
| Profanidade | zero |

### A régua "curto no que sente, generoso no que domina" — distribuição por parágrafo

Em palavras, pt-BR: **8 · 128 · 18 · 81 · 90 · 30 · 94 · 72**.

Os vales são os três batimentos emocionais; os picos são os dois blocos factuais mais caros (o rosto chegando e o playthrough inteiro):

- **8 palavras** — *"Eu tinha pernas desde terça. Não tinha cara."* (a ponte com a #3, seca, sem repetir o que ela já contou)
- **18 palavras** — *"Ter cara não é a mesma coisa que ter alguém para conversar. Isso eu só entendi depois, andando."* (o giro entre rosto e voz, o próprio argumento da lente, dito em uma frase)
- **30 palavras** — *"Doze dias de cara muda. Uma palavra... Era dele."* (o instante exato em que a distância contada fecha, e a voz que chega ainda não é a do narrador)

Os dois picos, **128** (a chegada do rosto: arte, decisão 2D, fim do placeholder) e **94** (o playthrough de 21/jul: cidade, NPC, arena, motor único, fechado às 20h02), são onde a peça paga o fato datado — a mesma régua que a #3 usou, generosa no que ela domina (o que aconteceu), curta no que ela sente (o que aquilo significou). O horário do segundo pico só entrou depois da conferência dos hashes: o commit de fecho (`cafd14c`) tem timestamp, e usá-lo aqui é a mesma lógica que a #3 aplicou às suas próprias horas — ancorar no registro real, não estimar.

### Fatos recusados — e por que cada um falhou no teste da contagem

O critério, per a lente aprovada: *"esse evento encurta a distância entre ter rosto e ter voz?"* Todos os itens abaixo são reais, datados e teriam matéria própria — só não passam neste teste, e por isso não entram, nem "para completar":

| Fato | Data | Por que falha o teste |
| :--- | :--- | :--- |
| Adoção do glintfx como motor de UI/HUD (ADR-010) | 01/jul | Infraestrutura de interface, não fala; tem **reportagem própria** (§10 da pauta) — citar aqui duplicaria a peça |
| M6 Áudio (AudioEngine, crossfade cidade↔arena) | 02-03/jul | O jogo ganha som, não alguém que responde — atmosfera, não voz |
| AssetSource (porteiro único de assets) | 04/jul | Cano de produção invisível ao teste "o mundo respondeu?" |
| Save v2 offline (AEAD, machine-binding) | 10/jul | Persistência de estado, não diálogo — não move rosto→voz |
| Motor de cartas techMagic fechado (ADR-016) | 14/jul | Reservado à Seção 9 (Cartas), e cartas são mecânica de combate, não fala do mundo |
| Arquitetura de conteúdo data-driven (ADR-019/020) | 19/jul | Estrutura interna de dados; não é o mundo falando, é como o mundo é organizado por dentro |
| Redesign "Tático Cockpit" (ADR-009) | 25/jun | Polimento visual da UI de batalha, não um NPC ganhando voz |
| O bug do flash do menu | 17/jul | Reservado à Galeria de Bugs (piada nerd primeiro, técnico depois — outro molde, outra seção) |
| M8/M9 decommission do Godot | 22/jul | Fora da janela aprovada (a Reportagem fecha em 21/jul, por decisão do líder) |

### Hashes citados — e onde resolvem

⚠️ **Esta sessão não teve, e continua sem ter, acesso a `Bash`/`gh api`.** Os cinco hashes abaixo foram
**conferidos pelo orquestrador**, com `gh api` direto contra o repositório — eu não os reconferi eu mesmo,
e registro isso para quem ler depois não confundir "conferido pela cadeia" com "conferido por este agente".
**Todos os cinco resolvem em `petrinhu/gusworld_legacy` — e SÓ lá.** ⛔ **Não resolvem no repo vivo do jogo**
(`petrinhu/gusworld` ou o nome que ele tiver hoje): esse foi refundado em 21/08/2026 e não carrega a história
de maio a julho. Quem for reconferir sem saber disso repete a investigação inteira do zero.

| Fato usado na peça | Data · hora local (GMT-3) | Hash | Assunto do commit (fonte) | Repositório onde resolve |
| :--- | :--- | :--- | :--- | :--- |
| Arena 2D via pipeline de arte, gate 2D-vs-3D fechado | 24/06/2026 | `96ebea2` | *"docs(design): arena = sprites 2D puros via PixelLab (gate 2D-vs-3D fechado)"* | `petrinhu/gusworld_legacy` |
| M5 BattleScreen incremento 3 (dá pra jogar) | 25/06/2026 | `d1b02e4` | *"feat(M5): BattleScreen incremento 3 - menu de verbos + conducao de turno (da pra jogar)"* | `petrinhu/gusworld_legacy` |
| M7-Diálogo — grafo POCO + parser + motor de runtime | 06/07/2026 | `81df01b` | *"feat(dialogue): M7-DIALOGO FORMATO+RUNTIME - grafo POCO + parser texto proprio + motor"* | `petrinhu/gusworld_legacy` |
| NPC Bertoldo Caím — redação final da primeira fala (`npc_intro`) | 06/07/2026 | `8e5664f` | *"feat(M7-DIALOGO): redacao final de voz do NPC Bertoldo (npc_intro)"* | `petrinhu/gusworld_legacy` |
| ★ M7 fechado — paridade jogável de ponta a ponta | 21/07/2026 · 20h02 local (23h02Z) | `cafd14c` | *"milestone(M7): paridade jogavel FECHADA (playthrough ao vivo, Gus, 2026-07-21)"* | `petrinhu/gusworld_legacy` |

★ **`cafd14c` é o achado que fechou a lacuna que eu tinha sinalizado.** O próprio assunto do commit nomeia
o playthrough ao vivo do Gus Dragon — é exatamente a âncora factual que a última cena da peça precisava, e
por isso o horário (**20h02**, já convertido de UTC para hora local — o GitHub devolve `23:02Z`, e a revista
sempre usa hora local) entrou na prosa (P7) e no comentário sobre os picos de contagem, acima.

**Nota de método, não repetir a investigação:** os primeiros quatro hashes já constavam em
`docs/editorial/LEVANTAMENTO-HISTORICO.md` (levantamento de `internal-auditor`, 2026-07-19, lido antes da
refundação) — mas eu não sabia, sem `gh api`, se ainda resolviam no repo pós-refundação. Agora sei que sim,
e sei o repositório certo (`gusworld_legacy`), sem precisar de nova varredura.
