...
| 15 | [`15-gus-no-bus.html`](15-gus-no-bus.html) | **A seção recorrente "Gus no Bus": as ideias que o Gus manda pelo bus git viram matéria.** O **centro é o ARTEFATO REAL** (decisão do líder 2026-07-17): o arquivo **`ideias1.txt`** inteiro que o menino subiu, **verbatim** (a grafia dele preservada: "Opcoes", "aleatorio", "depedendo", "voce", "nao"; o en-dash dele vem por entidade), num **papel pautado de caderno velho** (papel dentro do papel: pauta azul de 28px travada ao texto + margem vermelha + fita, tudo CSS, `white-space:pre-wrap`). Camadas: a **fala** (`gus@glyfesse>`) narra o destino das **4 ideias** (verdade da issue #1: foguinho vira carta, sinergia já existia, ataque em dupla depois, deckbuilding já cuidam) + o arquivo no caderno + o **pensamento `//`** sobre ter manchado. Um item **demo** ilustra o caso "não deu, sem spoiler" e a variação **sem mancha**. ★ Manchas com **borda orgânica** (filtro SVG feTurbulence + displacement), variam tipo e posição, **nunca sobre o texto** (zona livre + overflow:hidden). **Zero JS.** | ⏳ **pendente QA** |
